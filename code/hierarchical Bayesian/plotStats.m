...
for i = 1:5
    ...
    temp = subplot(1,5,i);
    scatter(norm_window1_std, norm_window2_std, '.', 'MarkerEdgeColor', colors{i});
    h = [h; temp];
...
set(h(1), 'Position', [0.05, 0.1, 0.15, 0.8]);
set(h(2), 'Position', [0.25, 0.1, 0.15, 0.8]);
set(h(3), 'Position', [0.45, 0.1, 0.15, 0.8]);
set(h(4), 'Position', [0.65, 0.1, 0.15, 0.8]);
set(h(5), 'Position', [0.85, 0.1, 0.15, 0.8]);
print_plot("1.png",11,2,800)
%%
...
for i = 1:5
    ...
    [f, xi] = ksdensity(correlation);
    plot(xi, f, "LineWidth", 2, "Color",  colors{i})
    xlabel("\gamma");
    ylabel("PDF");
    hold on;
    % hist(correlation, "PDF")
    title(legendNames{i});
    set(gca,"FontSize", 10, "FontName", "Times New Roman");
    grid on;
    box on;
