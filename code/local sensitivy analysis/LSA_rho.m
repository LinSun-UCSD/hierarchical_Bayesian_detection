    ...
    for mm = ii+1:5
        ...
        for jj = 1:length(range)
            ...
            for kk = 1:2
                ...
                temp = corr_coeff;
                temp(ii,mm) = range(jj) + delta(kk);
                temp(mm,ii) = range(jj) + delta(kk);
                Sigma = zeros(5, 5);
                for i = 1:5
                    for j = 1:5
                        Sigma(i, j) = temp(i, j) * std(i) * std(j);
                    end
                end
                ...
                res(ii, mm, jj, kk) = z_value;
            end

        end
    end
end
save("rho_LSA.mat", "res")
...
%%
function Sigma_psd = makePositiveDefinite(C)
eigVals = eig(C); % Compute eigenvalues
